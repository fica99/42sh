...
/*   Updated: 2019/10/19 19:21:59 by aashara-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_lexer		*load_lexer(void)
{
	...
	lexer = new_lexer();
	if (!(lexer->m_type = get_matrix("lexer/matrixs/types_matrix")))
		return (NULL);
	if (!(lexer->m_class = get_matrix("lexer/matrixs/classes_matrix")))
		return (NULL);
	if (!(lexer->m_union = get_matrix("lexer/matrixs/union_matrix")))
		return (NULL);
	if (!(lexer->m_priority = get_matrix("lexer/matrixs/priority_matrix")))
		return (NULL);
	if (!(lexer->m_generalization =
		get_matrix("lexer/matrixs/generalization_matrix")))
		return (NULL);
	return (lexer);
}
