...
/*   Updated: 2019/12/21 16:03:06 by ggrimes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
long long			calc_number(t_calc_tkns *s_tokens, size_t *index, t_calc_err *error)
{
	long long		result;
...
long long			calc_unary(t_calc_tkns *s_tokens, size_t *index, t_calc_err *error)
{
	if (!s_tokens || !calc_check_index(s_tokens, *index))
		return (calc_error(error, CALC_ERR_NULL_OR_IND));
